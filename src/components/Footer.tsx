      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
        ...
        <div id="footer-brand-col" className="flex flex-col items-start text-left">
          <div className="flex items-center gap-1 mb-4 -ml-4">
            <HariLogo className="h-12 w-auto" showSlogan={false} lightText={true} />
          </div>
        ...
        <div id="footer-areas-col">
          ...
          <div className="flex flex-wrap gap-1.5">
            {BANGALORE_AREAS.map((area, idx) => (
              <span
                key={idx}
                className="inline-block rounded bg-slate-900 border border-slate-800 px-2 py-1 text-[10px] font-mono text-slate-300 hover:text-[#e11d48] hover:border-red-500/30 transition cursor-default"
                title={`Premium Car mechanic nearby ${area.name}`}
              >
                {area.name}
              </span>
            ))}
